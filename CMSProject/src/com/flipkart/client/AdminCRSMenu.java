...
public class AdminCRSMenu {
	...
	private List<Student> viewPendingAdmissions() {
		...
		pendingStudentsList.forEach(student -> System.out.println(String.format("%20s | %20d | %20s | %20s", student.getUserId(), student.getStudentId(), student.getName(), student.getGender().toString())));
		return pendingStudentsList;
	}
	...
	private List<Course> viewCoursesInCatalogue() {
		...
		courseList.forEach(course -> System.out.println(String.format("%20s | %20s | %20s", course.getCourseCode(), course.getCourseName(), course.getInstructorId())));
		return courseList;
	}
}